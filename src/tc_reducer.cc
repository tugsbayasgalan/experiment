...
size_t OrderedCountHashJulian(const Graph &g){
  ...
  cilk_for(uintT i=0;i<num_nodes;i++) {
  ...
  cilk_for(long s=0;s<num_nodes;s++) {
  ...
  //#pragma omp parallel for schedule(dynamic, CHUNK_SIZE)
  cilk_for (uintT s=0;s<num_nodes;s++) {
    uintT d = hoffsets[s];
    if(d > 10000) {
      //#pragma omp parallel for schedule(dynamic, CHUNK_SIZE)
      cilk_for(auto j = g.out_neigh(s).begin(); j < g.out_neigh(s).end(); j++){
        TA[s].insert(*j);
      }
  ...
  cilk::reducer< cilk::op_add<int> > total(0);
  //int total = 0;
  //#pragma omp parallel for reduction(+ : total) schedule(dynamic, CHUNK_SIZE)
  cilk_for (NodeID u=0; u < g.num_nodes(); u++) {
    if (g.out_degree(u) > PAR_THRESHOLD) {
      //size_t localCount = 0;
      //cilk::reducer< cilk::opadd<size_t> > localCount(0);
      //auto v = g.out_neigh(u).begin();

      //#pragma omp parallel for reduction(+ : localCount)
      cilk_for (auto v = g.out_neigh(u).begin(); v < g.out_neigh(u).end(); v++) {
